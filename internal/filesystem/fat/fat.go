...
func (fat *FAT32) Type() filesystem.FileSystemType {
...
// ListDirectory on the reader-less FAT12/16/32 stub is an honest error: real
// directory parsing requires a reader and is implemented by FAT32Handler (see
// NewFAT32Handler). Returning canned entries here would fabricate evidence.
func (fat *FAT32) ListDirectory(path string) ([]filesystem.DirectoryEntry, error) {
	return nil, fmt.Errorf("FAT: directory parsing not yet implemented")
}
...
func (fat *FAT32) GetFileByPath(path string) (*filesystem.FileInfo, error) {
	return nil, fmt.Errorf("FAT32: file lookup requires directory parsing")
}

func (fat *FAT32) SearchFiles(rootPath string, predicate func(filesystem.FileInfo) bool) ([]filesystem.FileInfo, error) {
...
func init() {
	// These factories return the reader-less FAT32 stub, whose Open() validates
	// the boot sector and every directory/data operation returns an explicit
	// "not yet implemented" error — never fabricated entries. The real listing
	// path is NewFAT32Handler (reader-based), so DetectAndOpen can never hand
	// out a canned listing.
	filesystem.RegisterFileSystem(filesystem.FS_FAT32, func() filesystem.FileSystem {
		return &FAT32{}
	})
	filesystem.RegisterFileSystem(filesystem.FS_FAT16, func() filesystem.FileSystem {
		return &FAT32{}
	})
	filesystem.RegisterFileSystem(filesystem.FS_FAT12, func() filesystem.FileSystem {
		return &FAT32{}
	})
	// Reader-based handlers: OpenFileSystem (via NewHandler) opens a real
	// partition through NewFAT32Handler, which detects the actual FAT variant
	// (12/16/32) from the boot sector and serves it with the matching layout.
	filesystem.RegisterHandler(filesystem.FS_FAT32, func(r filesystem.Reader, startLBA, partitionSize uint64) (filesystem.FileSystem, error) {
		return NewFAT32Handler(r, startLBA, partitionSize)
	})
	filesystem.RegisterHandler(filesystem.FS_FAT16, func(r filesystem.Reader, startLBA, partitionSize uint64) (filesystem.FileSystem, error) {
		return NewFAT32Handler(r, startLBA, partitionSize)
	})
	filesystem.RegisterHandler(filesystem.FS_FAT12, func(r filesystem.Reader, startLBA, partitionSize uint64) (filesystem.FileSystem, error) {
		return NewFAT32Handler(r, startLBA, partitionSize)
	})
}
