...
//
// The cluster chain is still resolved eagerly (it is O(num clusters), not
// O(file bytes)); only the data reads are lazy. Every data byte comes from the
// same reader.ReadSectors exact-decompression path as ReadFile, so the red
// line holds: real on-disk data or an explicit error, never fabricated bytes.
func (h *FAT32Handler) OpenFile(path string) (io.ReadSeekCloser, error) {
	if h.reader == nil {
...
type fatFileReader struct {
	...
	// (ReadSectors), so concurrent ReadAt calls at different offsets do not
	// serialize on I/O.
	mu sync.Mutex
...
// newFATFileReader builds a lazy reader. An empty file returns a reader whose
// reads immediately hit io.EOF regardless of its (possibly 0) start cluster.
func newFATFileReader(h *FAT32Handler, start uint32, size uint64) (*fatFileReader, error) {
	r := &fatFileReader{
		h:           h,
		size:        int64(size),
...
// the FAT table on demand. It stops at an end-of-chain marker and rejects bad
// clusters, truncated chains, cycles and over-long chains.
//
...
func (r *fatFileReader) extendTo(clusterIdx int) error {
...
func (r *fatFileReader) readAt(p []byte, off int64) (int, error) {
	if off >= r.size {
		return 0, io.EOF
	}
	if len(p) == 0 {
	...
	for n < int(want) {
		...
		lba := r.h.clusterToLBA(cl)
		data, err := r.h.reader.ReadSectors(lba, uint64(r.h.sectorsPerCluster))
		if err != nil {
...
func (r *fatFileReader) Read(p []byte) (int, error) {
	if r.pos >= r.size {
		return 0, io.EOF
	}
...
func (r *fatFileReader) Seek(offset int64, whence int) (int64, error) {
	var abs int64
	switch whence {
	case io.SeekStart:
		abs = offset
...
func (r *fatFileReader) Close() error {
	return nil
}

var _ io.ReadSeekCloser = (*fatFileReader)(nil)
var _ filesystem.FileOpener = (*FAT32Handler)(nil)
