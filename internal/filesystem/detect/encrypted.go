...
func (bl *BitLocker) Type() filesystem.FileSystemType {
	return filesystem.FS_BITLOCKER
}
...
func (bl *BitLocker) ListDirectory(path string) ([]filesystem.DirectoryEntry, error) {
	if bl.encrypted {
		return nil, fmt.Errorf("BitLocker: volume is encrypted, cannot list without key")
	}
	// A reader-less / unencrypted fresh handler must never fabricate data.
	return nil, fmt.Errorf("BitLocker: directory parsing not yet implemented")
}
...
func (bl *BitLocker) GetFile(path string) ([]byte, error) {
	...
	return nil, fmt.Errorf("BitLocker: file reading not yet implemented")
}

func (bl *BitLocker) GetFileByPath(path string) (*filesystem.FileInfo, error) {
	return nil, fmt.Errorf("encrypted volume")
}

func (bl *BitLocker) SearchFiles(rootPath string, predicate func(filesystem.FileInfo) bool) ([]filesystem.FileInfo, error) {
	return nil, fmt.Errorf("encrypted volume")
}

func init() {
	filesystem.RegisterFileSystem(filesystem.FS_BITLOCKER, func() filesystem.FileSystem {
		return &BitLocker{}
	})
	filesystem.RegisterFileSystem(filesystem.FS_LUKS, func() filesystem.FileSystem {
		return &BitLocker{}
	})
}
